...
class DeepQNetwork(object):
    def __init__(
        ...
        self.s_ = tf.placeholder(tf.float32, [None, self.n_features], name='state_')  # input Next State
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            e1 = tf.layers.dense(self.s, units, tf.nn.relu, name='e1')
            self.q_eval = tf.layers.dense(e1, self.n_actions, name='q')

        # ------------------ build target_net ------------------
        with tf.variable_scope('target_net'):
            t1 = tf.layers.dense(self.s_, units, tf.nn.relu, name='t1')
            self.q_next = tf.layers.dense(t1, self.n_actions, name='t2')
